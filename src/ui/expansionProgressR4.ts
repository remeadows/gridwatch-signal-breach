...
export function loadExpansionR4Progress(storage: ProgressStorage | null = browserStorage()): ExpansionR4Progress {
  ...
  if (current?.schema === 1 && current.contentRevision === "expansion-1-r4") {
    return makeProgress(validId(current.highestUnlockedLevel, 25) ? current.highestUnlockedLevel : 1, ids(current.clearedLevels, 25));
  }
  ...
  const oldCleared = ids(old?.clearedLevels, 15);
  const cleared = oldCleared.map(mapRetainedLevel);
  const oldHighest = Math.max(validId(old?.highestUnlockedLevel, 30) ? old.highestUnlockedLevel : 1, ...oldCleared.map((id) => id + 1));
...
export function markExpansionR4LevelCleared(
  ...
  if (!validId(levelId, 25)) return current;
...
function mapRetainedAccess(id: number): number { return id > 15 ? 22 : mapRetainedLevel(id); }
